...
By \Cref{lemma:realization_over_Gm}(3), choose a reduced polynomial vector
$\vec f=(f_0,\dots,f_{r-1})$, with $f_j\in x^{-1}k[x^{-1}]$, such that
\[
p^{r-1-j}\deg_{x^{-1}}f_j<d\qquad(0\leq j<r),
\]
and such that
\[
\cP':=\Spec\Bigl(k[x,x^{-1}][\vec X]
    \big/\bigl(\wp\vec X\ominus\vec f\bigr)\Bigr)
\]
is a $\Z/p^r\Z$-torsor over $\bG_m$ with $\cP'_x\cong\cQ$. We fix this
realization for the remainder of the proof.
