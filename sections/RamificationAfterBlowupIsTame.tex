Throughout this chapter let $C$ be a projective smooth geometrically connected curve over $k$.
...
Our main goal in this chapter is to prove
\Cref{theorem:SymmetricPowerOfSheavesIsTamelyRamifiedReduction}. Let
$\cF$ be the rank-one $\Lambda$-local system occurring there and let
$\cP=\operatorname{Fr}(\cF)$ be its $G=\Lambda^\times$-torsor of frames.
By \Cref{prop:torsor_module_equivalence,definition:symmetric_power_local_system},
$\cP^{[\deg\ndls]}$ is the frame torsor of
$\cF^{[\deg\ndls]}$, and the two have identical ramification. Consequently,
it is enough to prove the following torsor formulation.
...
\section{\texorpdfstring{Ramification of $G$-Torsors}{Ramification of G-Torsors}}
...
\section{Proof of Theorem \ref{theorem:SymmetricPowerOfSheavesIsTamelyRamifiedReduction}}
...
\section{Extending To Product of Blowups}\label{subsection:ram_prod_analysis}
