...
\begin{lemma}
In the context of the preceding discussion, the morphism from $U'^{(d)}$ to $C'^{[d]}$ 
is unramified in codimension $1$ (see \Cref{definition:tamely_ramified_in_codim_1}).
\end{lemma}
\begin{proof}
    Let $\eta$ be the generic point of an irreducible component $W$ of the boundary $C'^{(d)} \setminus U'^{(d)}$.
    It is enough to show $\mathcal{O}_{C'^{[d]}, q(\eta)} \hookrightarrow \mathcal{O}_{C'^{(d)}, \eta}$ is weakly unramified (\Cref{definition:weakly_unramified_and_tamely_ramified_DVR}).
    Let $C' \setminus U' = \{x_1, \ldots, x_m \}$
