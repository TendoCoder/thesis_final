...
\begin{lemma}
    ...
    \end{tikzcd}
    \]
    Where $X$ and $X'$ are locally Noetherian schemes over a field $k$, $U \subset X$ and $U' \subset X'$ are dense open subsets,
    and $U' \to U$ is a $G$-torsor.
    Let 
    \[
        \begin{tikzcd}
       \mathrm{Sym}_k^d(U') \arrow[rd] \arrow[r, "q"] & U'^{(d)} \arrow[r] \arrow[d, "\pi"] & \mathrm{Sym}_k^d(X') \arrow[d] \\
       & \mathrm{Sym}_k^d(U) \arrow[r] & \mathrm{Sym}_k^d(X)
        \end{tikzcd}
    \]
    ...
    Let $(Z, \xi)$ be a prime divisor in $\mathrm{Sym}_k^d(X)$ such that $Z \cap \mathrm{Sym}_k^d(U) = \emptyset$ and let $(Z', \xi')$ be a prime divisor in $\mathrm{Sym}_k^d(X')$ such that $\pi(\xi') = \xi$.
