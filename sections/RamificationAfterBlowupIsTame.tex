...
Recall that $C'^{[d]}$ is the quotient of $C'^d$ by the finite group $\Xi = G_d \rtimes S_d$ (see \Cref{cor:sym-power-quotient}), 
...
%Then $A=R^{\Xi}$ where $\Xi = G_d \rtimes S_d$\footnote{Recall $G_d = \{ (g_1, \dots, g_d) \in G^d \mid g_1 g_2 \dots g_d = 1_G \}$} is the finite group acting on $C'^d$ as in \Cref{cor:sym-power-quotient}.
...
Next, we generalize by induction to the case of $G=\Z/p^r\Z$.
\begin{theorem}
...
\begin{proof}
    ...
    We proceed by induction on $r$. The base case $r=1$ is done by the previous theorem.
    Decompose $\cP \to U$ as $\cP \to \cP' \to U$ where $\cP \to \cP'$ is a $\Z/p\Z$-torsor and $\cP' \to U$ is a $\Z/p^{r-1}\Z$-torsor.

    \textbf{(1)} By the induction hypothesis, $\cP'^{[\deg \ndls]}$ is unramified at $\eta_\ndls$.

    Let $C'$ be the normal proper model of $\cP'$ viewed as a scheme, and let $\pi: C' \to C$ be the extension of $\cP' \to U$. 
    Let $\ndls' = \pi^{-1} (\ndls)$ be the pullback of $\ndls$ to $C'$.
    Let $\tilde{\ndls} $ be the pullback of $\ndls$ to $C'$.

    
    
    Then, following \Cref{lemma:torsor_unramified_codim_one}, and the discussion surronding it,
    with $U'$ being $\cP'$, we get that 
    
    
    \textbf{(2)} $\cP'^{(\deg \ndls)} \to \cP'^{[\deg \ndls]}  \hookrightarrow C'^{[\deg \ndls]}$ is unramified at codimension one points.

    And since $\cP \to \cP'$ is a $\Z/p\Z$-torsor, we can apply the previous theorem to deduce that $\cP^{[\deg \ndls]} \to \cP'^{(\deg \ndls)}$ is unramified at $\eta_\ndls$.
