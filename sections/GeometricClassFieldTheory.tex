...
\section{Etale Fundamental Groups and Tame Fundamental Groups}
We recall the definition and basic properties of the etale fundamental group, following stacks project 
\stackstag{0BQ6}
%% [REVIEW: unlabeled & never cited -- only feeds cor:etale_fundamental_group_smooth_proper below, which has no proof; either give the corollary a one-line proof citing this, or state only the corollary with the stacks tag]
...
\section{Generalized Picard Scheme}
...
%% [REVIEW: unlabeled -- Case 1 of the proof in ProofOfReducedTheorem.tex uses exactly this, but references \Cref{section:AbelJacobi} instead; add a label and reference the theorem directly]
\begin{theorem}
...
\section{Compactification of Blowup of Symmetric Powers of a Curve}\label{section:BlowupOfSymmetricPowerOfCurves}
...
\input{sections/RamificationOnProductsOfBlowups.tex}

\section{Proof of \Cref{theorem:SymmetricPowerOfSheafIsTamelyRamified}}
...
%% [REVIEW: label lemma:takeuchi_lemma is never referenced -- cor:etale_at_generic_point below depends on it silently (and has no proof); add 'Immediate from \Cref{lemma:takeuchi_lemma}' there. Note: the proof is nested inside the lemma environment]
...
%% [REVIEW: label lemma:tame_ramification_persists is never referenced -- it is applied silently in the paragraph right after ('Replacing ... we get'); add an explicit \Cref there]
...
Thus, by the following lemma, we conclude that $\boxP{\cP^{[\deg \mdls]}}{\cP^{[d - \deg \mdls]}} = p_1^{-1} \cP^{[\deg \mdls]} \otimes^G p_2^{-1} \cP^{[d - \deg \mdls]}$
     is tamely ramified at $\theta$ the generic point of $E_{\mdls} \times_k U^{(d-\deg \mdls)}$.

%% [REVIEW: unlabeled -- referenced only as 'the following lemma' in the preceding paragraph; works, but a label would make cor:tame-ramification-symmetric-product's proof traceable]
\begin{lemma}
...
\section{\texorpdfstring{Proof of \Cref{theorem:SymmetricPowerOfSheafIsTamelyRamified}}{Proof of Theorem X}}
