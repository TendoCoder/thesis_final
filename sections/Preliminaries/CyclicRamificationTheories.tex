...
an element of $\varphi(H)$ does not change its class.
...
stated and do not reproduce its proof. References and the precise scope in
which it is used are recorded in \Cref{appendix:schmid_witt_reference}.
...
\begin{enumerate}
    ...
    $f_j\in x^{-1}k[x^{-1}]$. With
    $m_j:=\deg_{x^{-1}}f_j$, it has the following properties:
    \begin{enumerate}
...
$m_j=\deg_{x^{-1}}f_j$. The Schmid--Witt formula gives
\[
\max_j p^{r-1-j}m_j<d,
\]
