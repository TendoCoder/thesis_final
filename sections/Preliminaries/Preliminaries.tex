...
\begin{cor}\label{cor:sym-power-quotient}
    Under the assumptions of the proposition above, and since the $G_d$ and $S_d$ actions commute,
    we get the following qoutient map
    \[
    ...
    $\mathrm{Sym}_S^d(\cP)$ and the symmetric power of $\cP$ as a $G$-torsor by $\cP^{(d)}$. 
    So that the above qoutient map is also written as:
    $q: \mathrm{Sym}_S^d(\cP) \to \cP^{(d)}$
\end{cor}

\textcolor{red}{maybe make it obvios that a scheme as symmetric product is $ \mathrm{Sym}_S^d(X)$ and not $X^{(d)}$ or replace 
torsor to $\cP^{[d]}$ instead of $\cP^{(d)}$}
