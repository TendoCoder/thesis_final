...
\begin{cor}\label{cor:sym-power-quotient}
    Under the assumptions of the proposition above, and given that $G$ is abelian, there is a canonical morphism of $G$-torsors over $\mathrm{Sym}_S^d(X)$:
    \[
         q: \cP^{\times_S d} / S_d \to G_d \backslash \cP^{\times_S d} / S_d
    \]
    ...
    $\cP^{(d)}$,
    %$\mathrm{Sym}_S^d(\cP)$
    and the symmetric power of $\cP$ as a $G$-torsor by $\cP^{[d]}$. 
    So that the above qoutient map is also written as:
    $q: \cP^{(d)} \to \cP^{[d]}$.
    %$q: \mathrm{Sym}_S^d(\cP) \to \cP^{[d]}$.
\end{cor}
\begin{proof}
    The morphism $q$ is induced by the canonical morphism $\cP^{\times_S d} \to G_d \backslash \cP^{\times_S d}$, and the fact that the $S_d$-action on $G_d \backslash \cP^{\times_S d}$ is compatible with the $S_d$-action on $\cP^{\times_S d}$
    (Assuming $G$ is abelian).
\end{proof}
