...
\textcolor{red}{insert here something about the equivilance of how we arrive at both G-torsors, one time as
products d times and one time as $G_d$ qoutient}
...
\[
p_1^{-1}\cP \otimes^G \cdots \otimes^G p_d^{-1}\cP = G_d \backslash \cP^{\times_S d}
\]
...
\end{prop}
%the $S_d$-action on $G_d \backslash \cP^{\times_S d}$ is compatible with the $S_d$-action on $X^{\times_S d}$, so that we get a canonical morphism $\cP^{[d]} \to \mathrm{Sym}_S^d(X)$.

% %Note that there is a canonical map $\cP^{\times_S d}/S_d \to \cP^$, and the $S_d$-action on $G_d \backslash \cP^{\times_S d}$ is compatible with the $S_d$-action on $X^{\times_S d}$, so that we get a canonical morphism $\cP^{[d]} \to \mathrm{Sym}_S^d(X)$.
...
the condition  $\prod_{i=1}^d g_i = 1$ defining $G_d$ is invariant under permuting the coordinates.
Therefore, the left action of $S_d$ normalizes the action of $G_d$. 
The group generated by these two actions is the semidirect product $\Xi = G_d \rtimes S_d$, 
which is a finite group acting on the left on $\cP^d$.
Then the double qoutient $(G_d \backslash \cP^{\times_S d}) / S_d \cong S_d \backslash (G_d \backslash \cP^{\times_S d})$ 
...
\begin{cor}\label{cor:sym-power-quotient}
    ...
    or equivlantly
    $$ q: S_d \backslash \cP^{\times_S d} \to (G_d \backslash \cP^{\times_S d}) / S_d$$
...
%     Let $X, X'$ be schemes and assume $X' \to X$ is a $G$-torsor. 
%     Let $X'^{(d)} = X'^d/S_d$ be the symmetric power of $X'$ as a scheme, and let 
%     $X'^{(d)}_G = G_d \backslash X'^d/S_d$ be the symmetric power of $X'$ as a $G$-torsor. 
%     By \Cref{prop:prop:symmetric_power_torsor} and the fact that $G_d, S_d$ actions commute, 
%     there is a natural qoutient map: $\pi: X'^{(d)} \to X'^{(d)}_G$.    
% \end{Corollary}
