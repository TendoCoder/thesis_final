...
The latter two identifications follow from $(\cP_{G/H})^{\times_S d} \cong H^d \backslash \cP^{\times_S d}$ together with the fact that the preimage of $K_{G/H} \subseteq (G/H)^d$ under $G^d \twoheadrightarrow (G/H)^d$ equals $H^d + \KG$.
...
The four quotient schemes above exist and fit into a canonical commutative diagram of torsors
\[
\begin{tikzcd}[row sep=large, column sep=large]
    \cP^{[d]_H} \arrow[r, "H"] \arrow[d, "K_{G/H}"'] & (\cP_{G/H})^{(d)} \arrow[d, "K_{G/H}"] \\
    \cP^{[d]_G} \arrow[r, "H"'] \arrow[dr, "G"', bend right=20] & (\cP_{G/H})^{[d]} \arrow[d, "G/H"] \\
    & X^{(d)},
\end{tikzcd}
\]
where each label indicates the structure group of the corresponding torsor. Moreover, the upper square is Cartesian:
\[
...
\begin{proof}
That all morphisms are torsors with the indicated structure groups is obtained by applying \Cref{prop:symmetric_power_torsor} and \Cref{cor:sym-power-quotient} to the three torsors at hand:
\begin{itemize}
    \item Applied to the $G$-torsor $\cP \to X$, we obtain the $G$-torsor $\cP^{[d]_G} \to X^{(d)}$.
    \item Applied to the $H$-torsor $\phi: \cP \to \cP_{G/H}$ (taking $\cP_{G/H}$ in place of the base $X$), we obtain the $H$-torsor $\cP^{[d]_H} \to (\cP_{G/H})^{(d)}$.
    \item Applied to the $G/H$-torsor $\psi: \cP_{G/H} \to X$, we obtain the $G/H$-torsor $(\cP_{G/H})^{[d]} \to X^{(d)}$, together with the canonical $K_{G/H}$-torsor $(\cP_{G/H})^{(d)} \to (\cP_{G/H})^{[d]}$ of \Cref{cor:sym-power-quotient}.
\end{itemize}

\textcolor{red}{understand this until the end...}
The factorization $\cP^{[d]_G} \xrightarrow{H} (\cP_{G/H})^{[d]} \xrightarrow{G/H} X^{(d)}$ of the diagonal arrow is precisely the decomposition of \Cref{prop:quotient_torsors_finite}, applied to the $G$-torsor $\cP^{[d]_G} \to X^{(d)}$ and the subgroup $H \subseteq G$. Indeed, both $\cP^{[d]_G} \times^G (G/H)$ and $(\cP_{G/H})^{[d]}$ are realized as the admissible quotient $((H^d + \KG) \rtimes S_d) \backslash \cP^{\times_S d}$.

The vertical morphism $\cP^{[d]_H} \to \cP^{[d]_G}$ on the left is the canonical projection induced by the inclusion of $S_d$-stable subgroups $K_H \rtimes S_d \subseteq \KG \rtimes S_d$ in $G^d \rtimes S_d$. Its structure group is therefore
\[
(\KG \rtimes S_d)/(K_H \rtimes S_d) \cong \KG/K_H,
\]
which by \Cref{eq:summation_kernels_ses} is isomorphic to $K_{G/H}$.

To prove the upper square is Cartesian, set $F := \cP^{[d]_G} \times_{(\cP_{G/H})^{[d]}} (\cP_{G/H})^{(d)}$.
The morphism $\cP^{[d]_G} \to (\cP_{G/H})^{[d]}$ is an $H$-torsor: the structure group $H \subseteq G$ acts on $\cP^{[d]_G}$ via the restriction of the $G$-action, while the $G$-action on $(\cP_{G/H})^{[d]}$ factors through $G/H$, so that $H$ acts trivially on the target. Base-changing this $H$-torsor along the $K_{G/H}$-torsor $(\cP_{G/H})^{(d)} \to (\cP_{G/H})^{[d]}$ exhibits $F$ as an $H$-torsor over $(\cP_{G/H})^{(d)}$.

The universal property of the fiber product furnishes a canonical morphism $\cP^{[d]_H} \to F$ over $(\cP_{G/H})^{(d)}$, and it is $H$-equivariant since the $H$-actions on both source and target are induced by the diagonal $H$-action on $\cP^{\times_S d}$.
Any morphism of $H$-torsors is an isomorphism, so $\cP^{[d]_H} \cong F$ and the upper square is Cartesian.
\end{proof}

\Cref{prop:torsor_tower_diagram} encodes two distinct factorizations of $\cP^{[d]_H} \to X^{(d)}$, which we shall refer to as the two \emph{torsor towers}:

\begin{description}
    \item[The $H$-symmetric tower,] obtained by traversing the top edge and the right column of the diagram:
    \[
    \cP^{[d]_H} \xrightarrow{\;H\;} (\cP_{G/H})^{(d)} \xrightarrow{\;K_{G/H}\;} (\cP_{G/H})^{[d]} \xrightarrow{\;G/H\;} X^{(d)};
    \]
    \item[The $G$-symmetric tower,] obtained by traversing the left column and the bottom row of the diagram:
    \[
    \cP^{[d]_H} \xrightarrow{\;K_{G/H}\;} \cP^{[d]_G} \xrightarrow{\;H\;} (\cP_{G/H})^{[d]} \xrightarrow{\;G/H\;} X^{(d)}.
    \]
\end{description}

The Cartesian property in \Cref{prop:torsor_tower_diagram} expresses that the connection morphism $\cP^{[d]_H} \to \cP^{[d]_G}$ on the left is the base change of the $K_{G/H}$-torsor $(\cP_{G/H})^{(d)} \to (\cP_{G/H})^{[d]}$ along the structural map $\cP^{[d]_G} \to (\cP_{G/H})^{[d]}$.
Properties of the torsor $(\cP_{G/H})^{(d)} \to (\cP_{G/H})^{[d]}$ that are stable under base change therefore descend automatically to the connection morphism on the left.
