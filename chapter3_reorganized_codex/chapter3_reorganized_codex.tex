...
\]
Second, we show that in this full-modulus compactification the tame
ramification is in fact unramified, and hence that $\cF^{[d]}$ extends
uniquely to a local system on $\widetilde C_{\mdls}^{(d)}$. Finally, since
$\widetilde\Phi_d$ is a projective space bundle, it induces an isomorphism
on \'etale fundamental groups. The extended local system therefore descends
uniquely to $\PicCm[d]$, proving \Cref{thm:GCFT_reduced}.
...
Let $\Lambda$ be a finite ring of cardinality invertible in $k$, and let
...
H=\widetilde C^{(d)}_\mdls\setminus U^{(d)}
  =E_0\times_{C^{(d)}}\widetilde C^{(d)}_\mdls.
\]
\end{theorem}

\subsection{Compatibility Under Products}\label{subsection:ram_prod_analysis}
We first record the product-compatibility result needed to combine the point-supported ramification calculations of Chapter~2.
...
\subsection{From Point-Supported Moduli to the Full Boundary}
...
we get that the $G$-torsor $p_1^{-1} \cP^{[\deg \mdls]}$ ($\cP$ corresponds to $\cF$ under \Cref{prop:torsor_module_equivalence})
...
proved above to extension across the boundary. The essential input is that an
affine space has no nontrivial finite cover which is tame along the hyperplane
at infinity.

\begin{lemma}\label{lemma:ExtensionOfTamelyRamifiedSheaf}
    Let $\cF^{[d]}$ be a locally constant sheaf of free rank-one $\Lambda$-modules
    on $U^{(d)}$. If $\cF^{[d]}$ is tamely ramified along
    \[
        H=\widetilde C^{(d)}_\mdls\setminus U^{(d)},
    \]
...
\begin{proof}
    Put $r=d-\deg\mdls-g+1$, let $\xi$ be the generic point of
    $\PicCm[d]$, and set $\Omega=k(\xi)^{\mathrm{sep}}$. By Takeuchi's
    construction, after base change of the generic fiber to $\Spec\Omega$,
    the compactified Abel--Jacobi fiber and its boundary are
    \[
        \left(
        \widetilde C^{(d)}_{\mdls,\Omega},
        H_{\Omega}
        \right)
        \cong
        \left(
        ...
        \right),
    \]
    and the open fiber is $\mathbb A^r_{\Omega}$.

    Let $\cP^{[d]}$ be the $\Lambda^\times$-torsor of frames of
    $\cF^{[d]}$. Its restriction to the geometric generic open fiber is tame
    along the hyperplane at infinity. T\'oth proves
    \cite[Example~3.3.8 and Claim~4.2.3]{Toth2011} that
    \[
        \pi_1^t\!\left(
        \mathbb A^r_{\Omega};
        \mathbb P^{r-1}_{\Omega}
        \right)=1.
    \]
    Let $\bar\eta$ be the generic point of the hyperplane
    $H_\Omega\cong\mathbb P^{r-1}_\Omega$. The pullback of $\cP^{[d]}$ to
    $\mathbb A^r_\Omega$ is therefore a constant $\Lambda^\times$-torsor.
    It extends as the constant finite \'etale torsor over
    $\mathbb P^r_\Omega$, so the inertia group $I_{\bar\eta}$ acts
    trivially. Let $\eta$ be the generic point of $H$. The passage from
    $\eta$ to $\bar\eta$ gives an unramified extension of the corresponding
    henselian discrete valuation rings: it enlarges the residue field
    separably and leaves a uniformizer unchanged. Hence
    $I_{\bar\eta}\cong I_\eta$, and triviality of inertia can be checked
    after this base change. Thus $I_\eta$ acts trivially, which says precisely
    that $\cP^{[d]}$ is unramified at the generic point of $H$.

    The scheme $\widetilde C^{(d)}_\mdls$ is regular, and $H$ is the only
    codimension-one component of its complement $U^{(d)}$. The standard
    normalization-and-purity argument, using Zariski--Nagata purity
    (\stackstag{0BMB}), therefore extends $\cP^{[d]}$ to a finite \'etale
    $\Lambda^\times$-torsor on $\widetilde C^{(d)}_\mdls$.
    (Indeed, take the componentwise normalization of
    $\widetilde C^{(d)}_\mdls$ in the finite \'etale cover $\cP^{[d]}$.
    This normalization is finite because $\widetilde C^{(d)}_\mdls$ is of
    finite type over a field, hence Nagata. Its branch locus contains no
    codimension-one point: over $U^{(d)}$ the cover is \'etale, and at the
    generic point of $H$ it is unramified by the preceding paragraph.
    Purity forces every nonempty branch locus over a regular scheme to have
    a codimension-one component, so the branch locus is empty.) The associated
    rank-one $\Lambda$-module is the desired extension
    $\widetilde{\cF}^{[d]}$. It is unique because a dense open immersion into
    a normal connected scheme induces a surjection on \'etale fundamental
    groups.
\end{proof}

\begin{rem*}
    Tameness does not imply unramifiedness for an arbitrary local extension.
    Here the implication comes from the global fiber geometry: the relevant
    boundary is the hyperplane at infinity of the geometric generic affine
    Abel--Jacobi fiber, whose tame fundamental group is trivial. This is the
    geometric argument used in the tame case by T\'oth.
\end{rem*}
...
We work over $S=\Spec k$, where $k$ is perfect. By
\Cref{prop:torsor_module_equivalence}, it is equivalent first to prove the
...
$d$, there exists a unique, up to isomorphism, $G$-torsor $\cQ_d$ on
$\PicCm[d]$ such that
\[
...
\Cref{theorem:abel-jacobi-fibers} identifies
\[
\Phi_d\colon C^{(d)}\longrightarrow\PicC[d]
\]
as a projective space bundle. Hence
\Cref{cor:projective-bundle-fundamental-group} gives
\[
...
$\cP$. By \Cref{definition:symmetric_power_local_system}, the frame torsor
of $\cF^{[d]}$ is $\cP^{[d]}$. For $d$ sufficiently large,
...
\]
The equivalence of \Cref{prop:torsor_module_equivalence}, together with the
compatibility of frame torsors and symmetric powers from
\Cref{definition:symmetric_power_local_system}, gives
$\Phi_d^*\cG_d\cong\cF^{[d]}$. The same equivalence carries uniqueness of
$\cQ_d$ to uniqueness of $\cG_d$.
\end{proof}
